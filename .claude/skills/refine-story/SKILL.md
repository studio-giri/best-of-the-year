...
- **Mechanism-altitude** (invisible: token transport, endpoint shape, table layout) → the **ADR**, but only what [`FORMAT.md`](../../../docs/adr/FORMAT.md) admits: a real, hard-to-reverse trade-off, recorded as *the decision and its why* — never its full spec. Column lists, index SQL, status codes, and route strings live in the code and schema they describe, not the ADR (an ADR can be a paragraph). Replace the open question with a reference to the ADR rather than copying mechanism into the Story.
