...
- **Email delivery** goes through a `Mailer` Effect `Layer`, so handlers depend on an interface rather than a concrete provider and the send is a mockable seam. The default layer dispatches over SMTP (`nodemailer`) using react-email templates from the `@boty/emails` package; tests provide a capturing layer instead. Local development points SMTP at a Mailpit container.
...
- Consuming a recovery link classifies it (invalid / used / expired) and marks it consumed in separate SELECT and UPDATE steps rather than one atomic conditional update, so its single-use guarantee is **best-effort**: two simultaneous consumes of the same still-valid link could both mint a token. Accepted under the same low-stakes framing — recovery is additive, so a double mint is indistinguishable from recovering twice. The race-safe upgrade (fold the mark into `UPDATE … WHERE consumed_at IS NULL RETURNING`, treat zero rows as already-used) is left for if the stakes ever rise.
